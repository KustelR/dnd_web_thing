...
export default async function Chat({lng}: {lng: string}) {
  ...
  return (
    <div className='border-gray-500 border-2  min-w-60 max-w-fit p-2 rounded-lg shadow-md shadow-black h-full'>
        <h2>{t('chat_header')}</h2>
        <ol className='py-4 h-full' children={messages.map((message) => {
            return (
            <li className='border-b-2 border-gray-500 p-1 bg-opacity-0 hover:bg-black hover:bg-opacity-10' key={message.id}>
